...
stepic = 'https://stepik.org/lesson/732079/step/3?unit=733612'
...
with webdriver.Chrome(options=options) as driver:
    ...
    for pin in driver.find_element('class name', 'main').text.split():
        wait.until(ec.element_to_be_clickable(('id', 'check'))).click()
        (alert := wait.until(ec.alert_is_present())).send_keys(pin)
        alert.accept()
