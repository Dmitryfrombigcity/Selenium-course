...
stepic = 'https://stepik.org/lesson/732079/step/1?unit=733612'
...
with webdriver.Chrome(options=options) as driver:
    ...
    for item in wait.until(ec.presence_of_all_elements_located(('tag name', 'input'))):
        item.click()
        wait.until((ec.alert_is_present())).accept()
        if result := driver.find_element('id', 'result').text:
            break
